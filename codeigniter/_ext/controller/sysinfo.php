...
class Sysinfo extends CI_Controller {
  ...
  public function __construct()
  {
    ...
    $this->server = ftt_get_instance('ftt/util/server', 'library');

    if(!$this->server->isftt())
    {
      echo "<!-- not allowd ip address-->";
      die;
    }
  }
  ...
  {
    $aMethod = (new ReflectionClass('Sysinfo'))->getMethods();
    $aRemove = array('index', 'get_instance');

    foreach ($aMethod as $oCurMethod)
    {
      if (substr($oCurMethod->name, 0, 1) == '_' || in_array($oCurMethod->name, $aRemove))
        continue;

      $sCurLink = rtrim($_SERVER['REQUEST_URI'], '/').'/';
      echo '<a href="'.$sCurLink.$oCurMethod->name.'">'.$oCurMethod->name."</a><br>\n";
    }
  }
...
<?php
  ...
  {
    require APPPATH.'config/'.ENVIRONMENT.'/database.php'; // $db

    echo 'Hostname : '.gethostname().' (env : '.ENVIRONMENT.")\n<br>\n";

    if (!empty($db) && is_array($db))
    {
      $oDao = new Database_Test_Dao();

      foreach ($db as $sDSN => $aConnInfo)
      {
        if (empty($aConnInfo['port']))
        {
          if ($aConnInfo['dbdriver'] == 'mssql')
            $aConnInfo['port'] = 1433;
          else if ($aConnInfo['dbdriver'] == 'mssqli' || $aConnInfo['dbdriver'] == 'mssql') 
            $aConnInfo['port'] = 3306;
        }
        echo '<b>['.$sDSN.']</b> host : '.$aConnInfo['hostname'].', port : '.$aConnInfo['port']."\n<br>";

        try{
          $sock = @fsockopen($aConnInfo['hostname'], $aConnInfo['port'], $errno, $errstr, 1);
          if ($sock)
            echo "O----------> connection test : <font color=blue>success!</font>\n<br>";
          else
            echo "X**********> connection test : <font color=red>fail!!!</font>\n<br>";

          $aResult = $oDao->getTestVal($sDSN);
          if (!empty($aResult) && !empty($aResult['test']))
            echo "O----------> query test : <font color=blue>".$aResult['test']."</font>\n<br>";
          else
            echo "X**********> query test : <font color=red>fail!!!</font>\n<br>";
        } catch (Exception $e) {
  ...
  {
    $sConfigPath = APPPATH.'config/'.ENVIRONMENT.'/';

    echo 'Hostname : '.gethostname().' (env : '.ENVIRONMENT.")\n<br/>\n<br/>\n";

    $sCheckServer = $this->input->get('svr');
    echo '<form><input type=text size=70 name=svr value="'.$sCheckServer.'" /><input type=submit value="connection check" /> (ex: http://www.sample.com/test, 10.10.1.1:1234)</form>';

    if (!empty($sCheckServer))
    {
      if (substr($sCheckServer, 0, 4) == 'http')
      {
        $aConnInfo = parse_url($sCheckServer);
        if (empty($aConnInfo['port']))
        {
          if ($aConnInfo['scheme'] == 'https')
            $aConnInfo['port'] = 443;
          else
            $aConnInfo['port'] = 80;
        }
      ...
      {
        if (strstr($sCheckServer, ':'))
        {
          $aTmpData = explode(':', $sCheckServer);
          $aConnInfo['host'] = $aTmpData[0];
          $aConnInfo['port'] = $aTmpData[1];
        }
        else
        {
          $aConnInfo['host'] = $sCheckServer;
          $aConnInfo['port'] = 80; // default
        }
      }
      ...
      echo 'host : '.$aConnInfo['host'].', port : '.$aConnInfo['port']."\n<br>";

      $sock = @fsockopen($aConnInfo['host'], $aConnInfo['port'], $errno, $errstr, 1);
    ...
    if (empty($this->input->get('cfg')))
    {
      $this->_printTree($this->_dirTree($sConfigPath));
    }
    else
    {
      $sConfigData = file_get_contents($sConfigPath.$this->input->get('cfg'));
      echo '<textarea cols=120 rows=50>'.$sConfigData.'</textarea>';
    }
  }

  private function _dirTree($dir)
  {
    $d = dir($dir);
    while (false !== ($entry = $d->read())) 
    {
      if($entry != '.' && $entry != '..' && substr($entry, -4) != '.swp')
      {
        if(is_dir($dir.$entry))
          $arDir[$entry] = $this->_dirTree($dir.$entry.'/');
        else
  ...
  private function _printTree($array, $level=0)
  {
    foreach($array as $key => $value) 
    {
      if(is_array($value))
      {
        echo "<div style='float:left; padding-left: ".($level*20)."px;'>&nbsp;</div>".$key."<br/>\n";
        $this->_printTree($value, $level+1);
      }
      else
      {
        echo "<div style='float:left; padding-left: ".($level*20)."px;'>&nbsp;</div><a href=\"".$_SERVER['SCRIPT_URI']."?cfg=".urlencode(substr($value, strlen(APPPATH.'config/'.ENVIRONMENT.'/')))."\">".$key."</a><br/>\n";
      }
    }
  }
...
class Database_Test_Dao
{
  public function getTestVal($sDSN)
  {
    $CI =& get_instance();
    $CI->load->database($sDSN);
    $CI->db->db_debug = FALSE;

    $query = "select '".$CI->db->dbdriver." query ok' test";

    if ($cursor = $CI->db->query($query, array()))
    {
