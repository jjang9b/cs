<script type="text/javascript">
  function showTableList4Account(aData, sStatus, oXhr)
  {
    ...
    $( 'select[name="account_table"]' ).html('');
    $( 'select[name="tab_table_all[]"]' ).hide();
    $( "#column_list" ).html('');

    for (var k in aData)
    {
      $( 'select[name="account_table"]' ).append( '<option value="' + aData[k] + '">' + aData[k] + '</option>' );
  ...
  function showColumnList4Account( type, aData )
  {
    ...
    html += '<thead>';
    html += '<tr>';
    html += '<th class="bg-red">USN 컬럼(키)</th>';
    ...
    for (var k in aData)
    {
      ...
      html += '<input type="radio" name="usn['+ type +'][]" value=' + aData[k].column_name + (aData[k].is_primary == 'Y' ? ' checked' : '') + ' />';
      html += '</td>';
      html += '<td align=center>';
  ...
  $(document).ready(function() {
    ...
    $("select[name='database']").change( function( e ){ 
      ...
      ajax('GET', '/wiz/account/getTableList', $.param(oData), showTableList4Account, onError);
    });

    $("select[name='account_table']").change( function(){ 
      var database = $("select[name='database']").val()
        , table = $( this ).val();

      window._tmp_main_table = table;

      $.get( '/wiz/account/getColumnList', { database : database, table : table }, function( r ){
    ...
    $("button[name='go']").click( function( e ){ 
      ...
      } else if( $( 'select[name="account_table"] option:selected' ).length == 0 ) {
    ...
    <div class="col-xs-12">
      <div class="box box-gray">
        <div class="box-header">
          ...
          <small><b class="text-red">사용자 정보</b> 조회 페이지 생성</small></h3>
        </div>
        <form id="fo" name="fo" action="/wiz/account/create" method="post">
          <div class="box-body">
...
?>
            ...
            <label>3. 사용자 정보 메인 테이블</label>
            <div class="callout callout-default">
              <div id="d_table" class="form-group">
                <label>3-1. Account table : 메인 사용자 정보 테이블 선택</label>
                <label class="control-label" for="inputError" style="display:none">
                <i class="fa fa-times-circle-o"></i><small> 메인 테이블을 선택해 주세요.</small></label>
                <select name="account_table" size=10 class="form-control"></select>
              </div>

              <div class="form-group">
                <label>3-2. Account table 노출 Column 선택 : USN 컬럼 선택 / 노출 컬럼 선택 / 한글 노출 컬럼명 입력</label>
                <div id="column_list" class="box-body table-responsive">Account table (사용자 정보 메인 테이블)을 선택해 주세요.</div>
              </div>
            </div>

            <label>4. 아래쪽 TAB 으로 노출할 테이블 </label>
            <div class="callout callout-default">
              <div class="form-group">
                <label>4-1. 노출 테이블</label>
        ...
        <input type="hidden" name="htype" value="account" />
      </form>
  </div>
</div>
